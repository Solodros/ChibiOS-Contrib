...
static void crc_lld_serve_interrupt(CRCDriver *crcp, uint32_t flags) {
  ...
  if (crcp->rem_data_size) {
    ...
    /* Portable CRC code defined in the high level driver, note, it is a macro.*/
...
void crc_lld_start(CRCDriver *crcp) {
  ...
  switch(crcp->config->poly_size) {
    case 32:
      crcp->crc->CTRL |= CRC_CTRL_POLYSIZE_32;
      break;
    case 16:
      crcp->crc->CTRL |= CRC_CTRL_POLYSIZE_16;
      break;
    case 8:
      crcp->crc->CTRL |= CRC_CTRL_POLYSIZE_8;
      break;
    case 7:
      crcp->crc->CTRL |= CRC_CTRL_POLYSIZE_7;
      break;
    default:
  ...
  if (crcp->config->reflect_data) {
    crcp->crc->CTRL |= CRC_CTRL_REVID_3;
  }
...
#if CRC_USE_DMA == TRUE
  crcp->dmamode = AT32_DMA_CTRL_DTD_M2M    | AT32_DMA_CTRL_PINCM |
		          AT32_DMA_CTRL_PWIDTH_BYTE | AT32_DMA_CTRL_MWIDTH_BYTE  |
				  AT32_DMA_CTRL_DTERRIEN       | AT32_DMA_CTRL_FDTIEN |
				  AT32_DMA_CTRL_CHPL(AT32_CRC_CRC1_DMA_PRIORITY);
  {
    crcp->dmastp = dmaStreamAllocI(AT32_CRC_CRC1_DMA_STREAM,
                                                 AT32_CRC_CRC1_DMA_IRQ_PRIORITY,
...
uint32_t crc_lld_calc(CRCDriver *crcp, size_t n, const void *buf) {
  ...
  if (crcp->config->reflect_data != 0) {
    ...
    crcp->crc->CTRL = (crcp->crc->CTRL & ~CRC_CTRL_REVID_3) | CRC_CTRL_REVID_2;
    while(n > 1) {
  ...
  if (crcp->config->reflect_data != 0) {
    crcp->crc->CTRL = (crcp->crc->CTRL & ~CRC_CTRL_REVID_3) | CRC_CTRL_REVID_1;
  }
...
void crc_lld_start_calc(CRCDriver *crcp, size_t n, const void *buf) {
  /* The DMA can only handle max 65535 bytes per transfer
