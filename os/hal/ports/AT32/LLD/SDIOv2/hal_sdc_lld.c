...
#define SDIO_DIV(n)             ((n & 0xFF) | (((n >> 8) & 0x3) << 15))
...
#if AT32_USB_USE_SDIO1 || defined(__DOXYGEN__)
SDCDriver SDCD1;
#endif
/** @brief SDCD2 driver identifier.*/
#if AT32_USB_USE_SDIO2 || defined(__DOXYGEN__)
SDCDriver SDCD2;
#endif
...
#endif
  div = sdcp->config->slowdown + ((AT32_HCLK + f - 1U) / f);
  if (div == 1U) {
...
__STATIC_FORCEINLINE uint32_t sdc_lld_get_timeout(SDCDriver *sdcp,
  ...
  return (((AT32_HCLK / (div * 2U)) / 1000U) * ms);
}
...
#if AT32_USB_USE_SDIO2
  sdcObjectInit(&SDCD2);
  SDCD1.thread = NULL;
  SDCD1.dma    = NULL;
  SDCD1.sdio   = SDIO2;
  nvicEnableVector(AT32_SDIO2_NUMBER, AT32_SDC_SDIO2_IRQ_PRIORITY);
#endif
}
...
void sdc_lld_start(SDCDriver *sdcp) {
  ...
  if (sdcp->state == BLK_STOP) {
    ...
    dmaSetRequestSource(sdcp->dma, AT32_SDC_SDIO_DMAMUX_CHANNEL, AT32_DMAMUX_SDIO);
      ...
      dmaSetRequestSource(sdcp->dma, AT32_DMAMUX_SDIO1);
#endif
...
#if AT32_USB_USE_SDIO2
  sdcp->dmamode = AT32_DMA_CCTRL_CHPL(AT32_SDC_SDIO2_DMA_PRIORITY) |
                  AT32_DMA_CCTRL_PWIDTH_WORD |
                  AT32_DMA_CCTRL_MWIDTH_WORD |
                  AT32_DMA_CCTRL_MINCM;
  ...
  /* If in stopped state then clocks are enabled and DMA initialized.*/
  if (sdcp->state == BLK_STOP) {
    sdcp->dma = dmaStreamAllocI(AT32_SDC_SDIO2_DMA_STREAM,
                                AT32_SDC_SDIO2_IRQ_PRIORITY,
                                NULL,
                                NULL);
    osalDbgAssert(sdcp->dma != NULL, "unable to allocate stream");

#if AT32_DMA_SUPPORTS_DMAMUX
    dmaSetRequestSource(sdcp->dma, AT32_SDC_SDIO2_DMAMUX_CHANNEL, AT32_DMAMUX_SDIO2);
#endif
