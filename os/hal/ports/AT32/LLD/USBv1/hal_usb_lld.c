...
#if AT32_USB_USE_USB1 || defined(__DOXYGEN__)
USBDriver USBD1;
#endif
...
static uint32_t usb_pm_alloc(USBDriver *usbp, size_t size) {
  ...
  osalDbgAssert(usbp->pmnext <= AT32_USB_PMA_SIZE, "PMA overflow");
...
static size_t usb_packet_read_to_buffer(usbep_t ep, uint8_t *buf) {
  size_t i, n;
  at32_usb_descriptor_t *udp = USB_GET_DESCRIPTOR(ep);
  at32_usb_pma_t *pmap = USB_ADDR2PTR(udp->RXADDR0);
#if AT32_USB_USE_ISOCHRONOUS
  uint32_t epr = AT32_USB->EPR[ep];
...
static void usb_packet_write_from_buffer(usbep_t ep,
                                         ...
                                         size_t n) {
  at32_usb_descriptor_t *udp = USB_GET_DESCRIPTOR(ep);
  at32_usb_pma_t *pmap = USB_ADDR2PTR(udp->TXADDR0);
  int i = (int)n;
  ...
  if (EPR_EP_TYPE_IS_ISO(epr) && (epr & EPR_DTOG_TX))
    udp->TXCOUNT1 = (at32_usb_pma_t)n;
  else
    udp->TXCOUNT0 = (at32_usb_pma_t)n;
#else
  udp->TXCOUNT0 = (at32_usb_pma_t)n;
#endif
  ...
  while (i >= 16) {
    ...
    w  = *(buf + 0);
    w |= *(buf + 1) << 8;
    *(pmap + 0) = (at32_usb_pma_t)w;
    w  = *(buf + 2);
    w |= *(buf + 3) << 8;
    *(pmap + 1) = (at32_usb_pma_t)w;
    w  = *(buf + 4);
    w |= *(buf + 5) << 8;
    *(pmap + 2) = (at32_usb_pma_t)w;
    w  = *(buf + 6);
    w |= *(buf + 7) << 8;
    *(pmap + 3) = (at32_usb_pma_t)w;
    w  = *(buf + 8);
    w |= *(buf + 9) << 8;
    *(pmap + 4) = (at32_usb_pma_t)w;
    w  = *(buf + 10);
    w |= *(buf + 11) << 8;
    *(pmap + 5) = (at32_usb_pma_t)w;
    w  = *(buf + 12);
    w |= *(buf + 13) << 8;
    *(pmap + 6) = (at32_usb_pma_t)w;
    w  = *(buf + 14);
    w |= *(buf + 15) << 8;
    *(pmap + 7) = (at32_usb_pma_t)w;
...
#if AT32_USB1_MAPHP_NUMBER != AT32_USB1_MAPLP_NUMBER
#if AT32_USB_USE_ISOCHRONOUS
/**
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AT32_USB1_MAPHP_HANDLER) {
  uint32_t istr;
...
#endif /* AT32_USB_USE_ISOCHRONOUS */
#endif /* AT32_USB1_MAPLP_NUMBER != AT32_USB1_MAPHP_NUMBER */
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AT32_USB1_MAPLP_HANDLER) {
  uint32_t istr;
  USBDriver *usbp = &USBD1;
  ...
  if ((istr & ISTR_SUSP) != 0U) {
    AT32_USB->CNTR |= CNTR_FSUSP;
#if AT32_USB_LOW_POWER_ON_SUSPEND
    AT32_USB->CNTR |= CNTR_LP_MODE;
#endif
    _usb_suspend(usbp);
  }
...
#else
#if AT32_USB1_HP_NUMBER != AT32_USB1_LP_NUMBER
#if AT32_USB_USE_ISOCHRONOUS
/**
 * @brief   USB high priority interrupt handler.
 *
 * @isr
 */
OSAL_IRQ_HANDLER(AT32_USB1_HP_HANDLER) {
  uint32_t istr;
  USBDriver *usbp = &USBD1;
...
#endif /* AT32_USB_USE_ISOCHRONOUS */
#endif /* AT32_USB1_LP_NUMBER != AT32_USB1_HP_NUMBER */
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AT32_USB1_LP_HANDLER) {
  uint32_t istr;
  USBDriver *usbp = &USBD1;
...
#endif /* AT32_USB_USB1_IRQ_REMAP */
#endif /* AT32_USB_USE_USB1 */
...
void usb_lld_start(USBDriver *usbp) {
  ...
  if (usbp->state == USB_STOP) {
    /* Clock activation.*/
#if AT32_USB_USE_USB1
    if (&USBD1 == usbp) {

      osalDbgAssert((AT32_USBCLK >= (48000000U - AT32_USB_48MHZ_DELTA)) &&
                    (AT32_USBCLK <= (48000000U + AT32_USB_48MHZ_DELTA)),
      ...
      AT32_USB->CNTR = CNTR_FRES;

      /* Enabling the USB IRQ vectors, this also gives enough time to allow
         the transceiver power up (1uS).*/
#if AT32_USB_USB1_IRQ_REMAP
#if AT32_USB1_MAPHP_NUMBER != AT32_USB1_MAPLP_NUMBER
      nvicEnableVector(AT32_USB1_MAPHP_NUMBER, AT32_USB_USB1_MAPHP_IRQ_PRIORITY);
#endif
      nvicEnableVector(AT32_USB1_MAPLP_NUMBER, AT32_USB_USB1_MAPLP_IRQ_PRIORITY);
#else
#if AT32_USB1_HP_NUMBER != AT32_USB1_LP_NUMBER
      nvicEnableVector(AT32_USB1_HP_NUMBER, AT32_USB_USB1_HP_IRQ_PRIORITY);
#endif
      nvicEnableVector(AT32_USB1_LP_NUMBER, AT32_USB_USB1_LP_IRQ_PRIORITY);
#endif
...
#if AT32_USB_USB1_IRQ_REMAP
#if AT32_USB1_MAPHP_NUMBER != AT32_USB1_MAPLP_NUMBER
      nvicDisableVector(AT32_USB1_MAPHP_NUMBER);
#endif
      nvicDisableVector(AT32_USB1_MAPLP_NUMBER);
#else
#if AT32_USB1_HP_NUMBER != AT32_USB1_LP_NUMBER
      nvicDisableVector(AT32_USB1_HP_NUMBER);
#endif
      nvicDisableVector(AT32_USB1_LP_NUMBER);
#endif

      AT32_USB->CNTR = CNTR_PDWN | CNTR_FRES;
      crmDisableUSB();
    }
#endif
  }
...
void usb_lld_reset(USBDriver *usbp) {
  ...
  AT32_USB->BTABLE = BTABLE_ADDR;
  AT32_USB->ISTR   = 0U;
  AT32_USB->DADDR  = DADDR_EF;
...
void usb_lld_set_address(USBDriver *usbp) {
  ...
  AT32_USB->DADDR = (uint32_t)(usbp->address) | DADDR_EF;
}
...
usbepstatus_t usb_lld_get_status_out(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (AT32_USB->EPR[ep] & EPR_STAT_RX_MASK) {
...
usbepstatus_t usb_lld_get_status_in(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (AT32_USB->EPR[ep] & EPR_STAT_TX_MASK) {
...
void usb_lld_clear_out(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((AT32_USB->EPR[ep] & EPR_STAT_RX_MASK) != EPR_STAT_RX_VALID) {
...
void usb_lld_clear_in(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((AT32_USB->EPR[ep] & EPR_STAT_TX_MASK) != EPR_STAT_TX_VALID) {
